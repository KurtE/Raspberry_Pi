...
void DIYXBeeController::ControlInput(void)
{
    ...
    if (ReceiveXBeePacket(&g_diyp))
    {
        ...
        if (memcmp((void*)&g_diyp, (void*)&g_diypPrev, sizeof(g_diyp)) != 0)
        {
            ...
            if (g_fDebugOutput)
            {
                DBGSerial.print( g_diyp.s.wButtons, HEX);
                DBGSerial.print(" - ");
                DBGSerial.print(g_diyp.s.bRJoyLR, DEC);
                DBGSerial.print(" ");
                DBGSerial.print(g_diyp.s.bRJoyUD, DEC);
                DBGSerial.print(" ");
                DBGSerial.print(g_diyp.s.bLJoyLR, DEC);
                DBGSerial.print(" ");
                DBGSerial.print(g_diyp.s.bLJoyUD, DEC);
                DBGSerial.print(" - ");
                DBGSerial.print(g_diyp.s.bRSlider, DEC);
                DBGSerial.print(" ");
                DBGSerial.println(g_diyp.s.bLSlider, DEC);
            }
#endif
#endif
        }
        ...
        // OK lets try "0" button for Start.
                                                  //Start Button (0 on keypad) test
        if ((g_diyp.s.wButtons & (1<<0)) && ((g_diypPrev.s.wButtons & (1<<0)) == 0))
        {
        ...
        {
                                                  // A button test
            if ((g_diyp.s.wButtons & (1<<0xa)) && ((g_diypPrev.s.wButtons & (1<<0xa)) == 0))
            {
                MSound(1, 50, 2000);
                ...
                XBeeOutputStringF(F("Walking"));
                                                  ...
                                                  // B button test
            if ((g_diyp.s.wButtons & (1<<0xb)) && ((g_diypPrev.s.wButtons & (1<<0xb)) == 0))
            {
                MSound(1, 50, 2000);
                XBeePlaySounds(1, 50, 2000);
                ...
                XBeeOutputStringF(F("Body Translate"));
                                                  ...
                                                  // C button test
            if ((g_diyp.s.wButtons & (1<<0xc)) && ((g_diypPrev.s.wButtons & (1<<0xc)) == 0))
            {
                MSound(1, 50, 2000);
                ...
                XBeeOutputStringF(F("Body Rotate"));
                                                  ...
                                                  // D button test - Single Leg
            if ((g_diyp.s.wButtons & (1<<0xD)) && ((g_diypPrev.s.wButtons & (1<<0xd)) == 0))
            {
                MSound(1, 50, 2000);
                                                  // none
                if (g_InControlState.SelectedLeg==255)
                    g_InControlState.SelectedLeg=cRF;
                                                  //Double press to turn all legs down
                else if (bXBeeControlMode==SINGLELEGMODE)
                                                  //none
                    g_InControlState.SelectedLeg=255;
                bXBeeControlMode=SINGLELEGMODE;
                XBeeOutputStringF (F("Single Leg"));
            }
                                                  // E button test - Balance mode
            if ((g_diyp.s.wButtons & (1<<0xe)) && ((g_diypPrev.s.wButtons & (1<<0xe)) == 0))
            {
...
#ifdef OPT_GPPLAYER
                                                  // F button test - GP Player
            if ((g_diyp.s.wButtons & (1<<0xf)) && ((g_diypPrev.s.wButtons & (1<<0xf)) == 0))
            {
                                                  //F Button GP Player Mode Mode on/off -- SSC supports this mode
                if (g_ServoDriver.FIsGPEnabled())
                {
            ...
            iNumButton = 0;                       // assume no button
                                                  // buttons 1-9
            if ((g_diyp.s.wButtons & 0x3fe) && ((g_diypPrev.s.wButtons & 0x3fe) == 0))
            {
            ...
            // We will do slightly different here than the RC version as we have a bit per button
                                                  //1-8 Button Gait select
            if ((bXBeeControlMode==WALKMODE) && iNumButton  && (iNumButton <= NUM_GAITS))
            {
                    ...
                    abs(g_InControlState.TravelLength.y*2)<cTravelDeadZone)
                {
                    ...
                    XBeeOutputString(s_asGateNames[g_InControlState.GaitType]);
                }
            }
            ...
            if (g_diystate.cbPacketSize > PKT_MSLIDER)
                g_InControlState.BodyPos.y = SmoothControl((g_diyp.s.bMSlider / 2), g_InControlState.BodyPos.y, SmDiv);
                                                  ...
                                                  //Zenta test
                g_InControlState.BodyPos.y =  SmoothControl((g_diyp.s.bLPot*2/3), g_InControlState.BodyPos.y, SmDiv);
            ...
            //Leg lift height - Right slider has value 0-255 translate to 30-93
                                                  //Zenta trying 3 instead of 4
            g_InControlState.LegLiftHeight = 30 + g_diyp.s.bRSlider/3;
            ...
            if (bXBeeControlMode==WALKMODE)       // Kurt's Arduino version
            {
                if (g_diystate.cbPacketSize > PKT_MSLIDER)
                {
                    ...
                    g_InControlState.InputTimeDelay = 128 -  max( max( abs(g_diyp.s.bLJoyLR-128),  abs(g_diyp.s.bLJoyUD-128)),  abs(g_diyp.s.bRJoyLR-128)) + (128 -(g_diyp.s.bLSlider)/2);
                                                  ...
                                                  // Case for original DIY XBee with extra pots
                else if (g_diystate.cbPacketSize > PKT_LPOT)
                {
                    g_InControlState.TravelLength.x = -(g_diyp.s.bLJoyLR - 128);
                    g_InControlState.TravelLength.z = -(g_diyp.s.bLJoyUD - 128) ;
            ...
            if (bXBeeControlMode==ROTATEMODE)
            {
                if (iNumButton &&(iNumButton <=3))
                {
                    ...
                    //          XBeeOutputString(s_asLJoyUDNames[LjoyUDFunction]);
                }
            ...
            if (bXBeeControlMode == SINGLELEGMODE)
            {
                ...
                g_InControlState.SLLeg.z = SmoothControl((-(g_diyp.s.bRJoyUD-128)), g_InControlState.SLLeg.z , SmDiv);
                                                  //Need to check packetsize..
                g_InControlState.SLLeg.y = SmoothControl((-(g_diyp.s.bLJoyUD-128)), g_InControlState.SLLeg.y , SmDiv);
                g_InControlState.InputTimeDelay = 128 -  max( max( abs(g_diyp.s.bRJoyLR-128),  abs(g_diyp.s.bRJoyUD-128)),  abs(g_diyp.s.bLJoyLR-128)) + (128 -(g_diyp.s.bLSlider)/2);
            }
                ...
                if (g_ServoDriver.FIsGPSeqActive() )
                {
                    ...
                    if (bCurStep != g_bGPCurStepPrev)
                    {
                        ...
                        XBeeOutputString(szTemp);
                                                  ...
                                                  //1-9 Button Play GP Seq
                if (iNumButton>=1 && iNumButton<=9)
                {
                    if (!g_ServoDriver.FIsGPSeqActive() )
                    {
                        ...
                        {
                                                  // Tell user sequence started.
                            XBeeOutputStringF(F("Start Sequence"));
                            g_ServoDriver.GPStartSeq(GPSeq);
                                                  // Say we are not in modifiy speed modifier mode yet...
                            g_sGPSMController = 32767;
                            g_bGPCurStepPrev = 0xff;
                        }
                        else
                        {
                                                  // that sequence was not defined...
                            XBeeOutputStringF(F("Seq Not defined"));
                        }
                    }
                    else
                    {
