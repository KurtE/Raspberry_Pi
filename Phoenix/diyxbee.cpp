...
uint8_t APIRecvPacket(ulong Timeout)
{
    ...
    for (i = 0; i < wPacketLen; i++)
        ...
        bChksum = bChksum + g_diystate.bAPIPacket[i];
...
boolean ReceiveXBeePacket(PDIYPACKET pdiyp)
{
    ...
    ulong ulCurrentTime;
    ulong ulTimeDiffMS;
                                                  // Save away the previous state as this is the state if no new data...
    boolean _fPacketValidPrev = g_diystate.fPacketValid;
    ...
    for (;;)
    {
        ...
        else if (g_diystate.bAPIPacket[bDataOffset + 0] == XBEE_TRANS_CHANGED_DATA)
        {
            if ((cbRead > 2) && (cbRead  < sizeof(DIYPACKET)))
            {
                ...
                uint16_t wChangeMask = (g_diystate.bAPIPacket[bDataOffset + 1] << 8) + g_diystate.bAPIPacket[bDataOffset + 2];
                uint8_t idiyp = 0;
        ...
        else if (g_diystate.bAPIPacket[bDataOffset + 0] == XBEE_TRANS_NOTHIN_CHANGED)
        {
            ...
            g_diystate.fPacketValid = true;       // data is valid
                                                  // Was the last request forced???
            g_diystate.fPacketForced = g_diystate.fReqDataForced;
        ...
        {
                                                  // take care of warning, probably not needed
            wNewDL = (g_diystate.bAPIPacket[bDataOffset + 1] << 8) + g_diystate.bAPIPacket[bDataOffset + 2];
        ...
        {
                                                  // 1 byte version
            g_diystate.bTransDataVersion = g_diystate.bAPIPacket[bDataOffset + 1];
        }
        //-----------------------------------------------------------------------------
        // [XBEE_TRANS_NEW]
        //-----------------------------------------------------------------------------
        ...
        {
                                                  // take care of warning, probably not needed
            word w = (g_diystate.bAPIPacket[bDataOffset + 1] << 8) + g_diystate.bAPIPacket[bDataOffset + 2];
        ...
        else if (g_diystate.bAPIPacket[bDataOffset + 0] == XBEE_DEBUG_STRING)
        {
            ...
            digitalWrite(0, !digitalRead(0));
                                                  // need to check byte order
            XBDSerial.SetInputText(&g_diystate.bAPIPacket[bDataOffset + 1], cbRead);
    ...
    if (g_diystate.fTransReadyRecvd)
    {
        ...
        if (ulTimeDiffMS > CXBEETIMEOUTRECVMS)
        {
            ...
            if (g_fDebugOutput)
            {
                ...
                {
                    DBGSerial.print("XBEE Timeout ");
                    DBGSerial.println( ulTimeDiffMS, DEC);
        ...
        if (g_diystate.fReqDataPacketSent)
        {
            if ((ulCurrentTime-g_diystate.ulLastRequest) > CXBEEPACKETTIMEOUTMS)
            {
                ...
                _fNewPacketAvail = true;          // make sure it requests a new one
                                                  // make sure we send a new one...
                g_diystate.fReqDataPacketSent = false;
            }
        }
        ...
        if (!g_diystate.fSendOnlyNewMode || (g_diystate.fSendOnlyNewMode && _fNewPacketAvail))
        {
            ...
            if (!g_diystate.fReqDataPacketSent)
            {
                                                  // Request data Prompt (CmdType, ChkSum, Packet Number, CB extra data)
                SendXBeePacket(g_diystate.wAPIDL, g_diystate.bTransDataVersion? XBEE_RECV_REQ_DATA2 : XBEE_RECV_REQ_DATA, 0, 0);
                                                  // yes we have already sent one.
                g_diystate.fReqDataPacketSent = true;
                                                  // remember when we sent this...
                g_diystate.ulLastRequest = ulCurrentTime;
            }
        }
                                                  // Say the data is in the same state as the previous call...
        g_diystate.fPacketValid = _fPacketValidPrev;
    }
