  ...
  if (_fGPActive) {
    if (g_bGPCntSteps == 0xff) {
      ...
      if (_iSeq == 0xff) {  // User told us to abort
        ...
        SSCRead((byte*)abStat, sizeof(abStat), 10000, -1);  //    [GPStatSeq, GPStatFromStep, GPStatToStep, GPStatTime]
...
uint8_t ServoDriver::GPNumSteps(void)          // How many steps does the current sequence have
{
  ...
  SSCSerial.println(F(";2"));
  cbRead = SSCRead((byte*)&wSeqStart, sizeof(wSeqStart), 10000, -1);  // Try to get the pointer to sequence
  ...
  if ((cbRead == sizeof(wSeqStart)) && (wSeqStart != 0) && (wSeqStart != 0xffff)) {
    ...
    cbRead = SSCRead((byte*)&bGPCntSteps, sizeof(bGPCntSteps), 10000, -1);  // Try to get the pointer to sequence
  }
  return bGPCntSteps;
}
