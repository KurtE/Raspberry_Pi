...
void Adafruit_ILI9341::fillScreen(uint16_t color) {
  // Lets start off by simply addressing memory
  register uint8_t *pfbtft_pixel = _fbtft;
  while (pfbtft_pixel < (_fbtft+sizeof(_fbtft))) {
    *pfbtft_pixel++  = (color>>8) & 0xff;
    *pfbtft_pixel++ = color & 0xff;
  }
}
...
void Adafruit_ILI9341::fillRect(int16_t x, int16_t y, int16_t w, int16_t h,
  ...
  // Lets start off by simply addressing memory
  register uint8_t *pfbtft_pixel_row = &_fbtft[(y*_width + x) * 2];
  for (;h>0; h--) {
    register uint8_t *pfbtft_pixel = pfbtft_pixel_row;
    for (int i = 0 ;i < w; i++) {
      *pfbtft_pixel++  = (color>>8) & 0xff;
      *pfbtft_pixel++ = color & 0xff;
    }
    pfbtft_pixel_row += (_width * 2);
  }
}
...
void Adafruit_ILI9341::update(void) {
  ...
  for (int i = 0; i < CNT_SPI_WRITES; i++) {
    mraa_spi_transfer_buf(SPI, prxData, NULL, CNT_BYTES_PER_WRITE);
    prxData += CNT_BYTES_PER_WRITE;
  }
  CSHigh();
  spi_end();
}
...
void Adafruit_ILI9341::writeRect(int16_t x, int16_t y, int16_t w, int16_t h, uint16_t *pcolors) 
{
  ...
  for (;h>0; h--) {
    register uint8_t *pfbtft_pixel = pfbtft_pixel_row;
    for (int i = 0 ;i < w; i++) {
      *pfbtft_pixel++  = (*pcolors>>8) & 0xff;
      *pfbtft_pixel++ = *pcolors++ & 0xff;
    }
    pfbtft_pixel_row += (_width * 2);
  }
}
