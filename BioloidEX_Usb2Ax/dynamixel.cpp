...
void dxl_tx_packet()
{
	...
	if( gbInstructionPacket[INSTRUCTION] != INST_PING
		...
		&& gbInstructionPacket[INSTRUCTION] != INST_SYNC_READ   // Below ones added for USB2AX...
		&& gbInstructionPacket[INSTRUCTION] != INST_READ_POSE
		&& gbInstructionPacket[INSTRUCTION] != INST_POSE_IDS
		&& gbInstructionPacket[INSTRUCTION] != INST_POSE_MASK
		&& gbInstructionPacket[INSTRUCTION] != INST_POSE_ABORT
        )
	{
		gbCommStatus = COMM_TXERROR;
		giBusUsing = 0;
...
{
//    printf("dxl_read_byte %d %d", id, address);
	while(giBusUsing);
	...
	dxl_txrx_packet();
//    printf(" %d return = %d\n\r", (int)gbStatusPacket[PARAMETER], gbCommStatus);
...
{
//    printf("dxl_write_byte %d %d %d", id, address, value);
	while(giBusUsing);
